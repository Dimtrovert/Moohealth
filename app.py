...
# ==========================================
# Inisialisasi Data Memori (Aman untuk Vercel)
# ==========================================
...
# Menyimpan riwayat data untuk grafik dan ekspor CSV
history_data = []
...
def restrict_dashboard_access():
    allowed_routes = ['login', 'static', 'receive_cow_sensor']
    # Jangan blokir endpoint ESP32 agar sensor tetap bisa mengirim data tanpa login
    if request.endpoint == 'receive_cow_sensor':
        return None
...
def login():
    if request.method == 'POST':
        ...
        # Simple authentication logic (Ganti dengan validasi DB jika untuk produksi)
...
def download_csv():
    """Menghasilkan file CSV secara on-the-fly dari memori tanpa menyentuh file system"""
    ...
    for row in data_to_export:
        cw.writerow([row.get('timestamp', ''), row.get('suhu', ''), row.get('jantung_bpm', ''), row.get('status', '')])
...
# ==========================================
# Dummy Data Generator (Hanya berjalan di lokal)
# ==========================================
...
if __name__ == '__main__':
    # Thread dummy hanya dieksekusi saat dijalankan manual (python app.py)
    # Vercel tidak akan pernah mengeksekusi blok __main__ ini.
    print("Menjalankan server dalam mode pengembangan lokal...")
